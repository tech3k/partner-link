...
beforeEach(() => {
    jest.spyOn(CreditSearch.prototype, 'getJwt').mockImplementation(async () => await 'auth-token');
});
...
describe('CreditSearch: checkAddress', () => {
    ...
    it('should return credit search adress results', async () => {
        jest.spyOn(CreditSearch.prototype, 'soapRequest')
            .mockImplementation(async () => await `<?xml version="1.0" encoding="UTF-8" standalone="yes" ?>
...
<soap:Envelope xmlns:soap="http://www.w3.org/2003/05/soap-envelope/" soap:encodingStyle="http://www.w3.org/2003/05/soap-encoding">
    <soap:Body>
        ...
        expect(service.checkAddress(data)).rejects.toThrowError('Address 12, M1 6NG was not found.');
    });
});
...
describe('CreditSearch: checkMultipleAddresses', () => {
    it('should throw error when checking multiple addresses', () => {
        ...
        expect(service.checkMultipleAddresses(data)).rejects.toThrowError(PartnerLinkError);
        expect(service.checkMultipleAddresses(data)).rejects.toThrowError('No more than 3 addresses can be credit searched.');
    });
...
<soap:Body>
    <GetLightSearchCreditReportHTMLResponse xmlns="http://searchlink.co.uk/">
        ...
        const service = new CreditSearch({debug: true} as PartnerLinkCredentials);
        const resp = new CreditSearchPersonResult();
        resp.id = 451912;
        resp.creditors = [];
        resp.report = '';
    ...
    it('should throw error', async () => {
        jest.spyOn(CreditSearch.prototype, 'soapRequest').mockImplementation(async () => {
